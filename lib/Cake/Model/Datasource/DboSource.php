...
/**
 * Filter for renderStatement
 *
 * @var null|callable
 */
	protected $sqlFilterCallback = null;
	...
	public function renderStatement($type, $data) {
		...
		switch (strtolower($type)) {
			...
			case 'delete':
				...
				return $this->applySqlFilter("DELETE {$alias} FROM {$table} {$aliases}{$conditions}");
			case 'schema':
 ...
 * @return bool is filter successfully installed
 */
    public function setSqlFilter($callback) {
		if($callback !== null && !is_callable($callback)) {
			return false;
		}
		$this->sqlFilterCallback = $callback;
		return true;
	}

/**
 * Applies filter provided by user to sql generated by renderStatement
 *
 * @param  $sql original sql
 * @return string filtered sql
 */
	protected function applySqlFilter($sql) {
		if($this->sqlFilterCallback !== null) {
			return call_user_func($this->sqlFilterCallback, $sql);
		} else {
			return $sql;
		}
	}
